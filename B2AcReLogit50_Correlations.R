...
TablePath <- "/storage/work/ejm6007/B2AcReLogit50/B2AcReLogit50_Tables"
...
#Load matrices
B2AcReLogit50 <- read_csv("/storage/work/ejm6007/B2AcReLogit50/B2AcReLogit50_Tables/B2AcReLogit50.csv")
B2AcReLogit50InitPrm <-  read_csv("/storage/work/ejm6007/B2AcReLogit50/B2AcReLogit50_Tables/B2AcReLogit50InitPrm.csv")

# B2AcReLogit50 <- read_csv("B2AcReLogit50.csv")
# B2AcReLogit50InitPrm <-  read_csv("B2AcReLogit50InitPrm.csv")

#Find for which runs I need to add correlations
B2AcReLogit50ToAddCorr<-B2AcReLogit50[which(B2AcReLogit50$CorrelationsRed!= "NA - NA"),]

ID<-B2AcReLogit50ToAddCorr$Run

for (z in 86:length(ID)){
  #Select the initial prm values
  initprms<-B2AcReLogit50InitPrm[which(B2AcReLogit50InitPrm$x==ID[z]), ]
  modelresults<-B2AcReLogit50[which(B2AcReLogit50$Run==ID[z]), ]
  ...
  for(l in 1:length(corrcurrent)){
    ...
    #load project
    loadProject(projectFile="/storage/work/ejm6007/B2AcReLogit50/B2AcReLogit50_2.mlxtran")
    ...
    image_name <- paste("VPC_B2AcReLogit50_", i, k, names[j],"C", l, sep = "")
    ggsave(filename = file.path(output_dir_VPC, paste0(image_name, ".jpg")),
           plot = m1, device = "jpeg")
    
    #Plot fitted curves
    m2<-plotIndividualFits(settings = list(xlab="Time (Days)", ylab="Log10 Viral Load",
                                           ylim=c(0, 6.5)))
    image_name <- paste("Fits_B2AcReLogit50_", i, k, names[j], "C", l, sep = "")
    ggsave(filename = file.path(output_dir_Fits, paste0(image_name, ".jpg")),
           plot = m2, device = "jpeg")
    
    # Plot Distribution of parameter values
    m3 <- plotParametersDistribution()
    image_name <- paste("PrmDist_B2AcReLogit50_", i, k, names[j],"C", l, sep = "")
    ggsave(filename = file.path(output_dir_PrmDist, paste0(image_name, ".jpg")),
           plot = m3, device = "jpeg")
    
    #save the table
    write.csv(tabestimates,  file.path(TablePath,"B2AcReLogit50CovariatesAbsLRCorr2.csv"))
    write.csv(initprmvalues,  file.path(TablePath,"B2AcReLogit50CovariatesAbsLRInitPrmCorr2.csv"))
    write.csv(indivprmvalues,  file.path(TablePath,"B2AcReLogit50CovariatesAbsLRIndivPrmCorr2.csv"))
