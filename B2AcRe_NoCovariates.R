  ...
  output_dir_VPC <-"/storage/work/ejm6007/B2AcRe/B2AcRe_Tables"
  ...
  i=0
  for (j in 1:5){
    loadProject(projectFile="/storage/work/ejm6007/B2AcRe/B2AcRe_.mlxtran")
    ...
    image_name <- paste("VPC_B2AcRe_0", names[j], sep = "")
    ...
    m2<-plotIndividualFits(settings = list(xlab="Time (Days)", ylab="Log10 Viral Load",
                                           ylim=c(0, 6.5)))
    image_name <- paste("Fits_B2AcRe_0", names[j], sep = "")
    ...
    image_name <- paste("PrmDist_B2AcRe_0", names[j], sep = "")
    ggsave(filename = paste0(image_name, ".jpg"), plot = m3, device = "jpeg")
    
    #save the table
    write.csv(tabestimates, file="B2AcReNoCovariates.csv")
    write.csv(initprmvalues, file="B2AcReNoCovariatesInitPrm.csv")
    write.csv(indivprmvalues, file="B2AcReNoCovariatesIndivPrm.csv")
  }
  ...
  for(i in 1:length(c_values)){
    ...
    loadProject(projectFile="/storage/work/ejm6007/B2AcRe/B2AcRe_.mlxtran")
      ...
      #Plot VPC
      m1<-plotVpc(settings = list(xlab="Time (Days)", ylab="Log10 Viral Load"))
      image_name <- paste("VPC_B2AcRe_", i, names[j], sep = "")
      ...
      m2<-plotIndividualFits(settings = list(xlab="Time (Days)", ylab="Log10 Viral Load",
                                             ylim=c(0, 6.5)))
      image_name <- paste("Fits_B2AcRe_", i, names[j], sep = "")
      ggsave(filename = file.path(output_dir_PrmDist, paste0(image_name, ".jpg")),
             plot = m2, device = "jpeg")
      
      # Plot Distribution of parameter values
      m3 <- plotParametersDistribution()
      image_name <- paste("PrmDist_B2AcRe_", i, names[j], sep = "")
      ggsave(filename = file.path(output_dir_PrmDist, paste0(image_name, ".jpg")),
             plot = m3, device = "jpeg")
